...
function process_text(page_meta) {
    ...
    chrome.extension.sendMessage({ type: 'chromex.dispatch', payload: { type: 'PROCESS_TEXT_RESULT', payload: { status: t.length > 200 ? true : false, url: document.location.href, text: t, } } });
    if (t.length > 200) {
        ajax_get_UrlNlpInfo(document.location.href, function (data) {
            ...
            if (data.is_known == true) {
                ...
                chrome.extension.sendMessage({ type: 'chromex.dispatch', payload: { type: 'NLP_INFO_UNKNOWN', payload: { url: document.location.href, status: false, } } });
                nlp_calais(page_meta, t, document.location);
            }
        }, function (error) {
            chrome.extension.sendMessage({ type: 'chromex.dispatch', payload: { type: 'NLP_INFO_ERROR', payload: { url: document.location.href, error: error, } } });
        });
    } else {
