...
$(document).ready(function () {
  ...
  //document.addEventListener("DOMContentLoaded", function (event)
  {
    ...
    if (document.location.href != sessionStorage["mm_cs_text_haveFiredDocReady_" + (mm_app_uuid && mm_app_uuid())]) {
        ...
        if (cslib_isYouTubeSite()) {
          if (cslib_isYouTubeWatch()) {
            ...
            if (comments_disabled.length > 0 ||
              ...
              log.info("YT -- comments disabled");

              get_page_metadata(true, function (error, page_meta) {
                if (error) {
                  StackTrace.fromError(error).then(cslib_errorStack).catch(cslib_errBack);
                  chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'PAGE_META_ERROR', payload: { url: remove_hash_url(document.location.href), page_meta: page_meta, } } });
                } else {
                  chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'PAGE_META', payload: { url: remove_hash_url(document.location.href), page_meta: page_meta, } } });
              ...
              log.info("YT -- comments enabled; waiting for load (2)...");
      ...
      log.info("%c mm_cs_text_haveFiredDocReady // NOP.", cs_log_style_hi);
    }
  }
  //);
});
...
function process_text(page_meta) {
  ...
  //
  if (cslib_isYouTubeWatch()) {
    log.info(" *** USING YOUTUBE/WATCH HANDLER ***");
  ...
  chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'PROCESS_TEXT_RESULT', payload: { status: t.length > 200 ? true : false, url: remove_hash_url(document.location.href), text: t, } } });
    ...
    if (detectLang === 'eng') {
      ajax_get_UrlNlpInfo(remove_hash_url(document.location.href), function (data) {
        ...
        if (data.is_known == true && data.has_calais_info == true) {
          ...
          chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'NLP_INFO_KNOWN', payload: { lang: detectLang, url: remove_hash_url(document.location.href), status: true, page_meta: page_meta, topics: data.topics, suggestions: data.suggestions } } });
          chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'GENERATE_SHARE_TOPICS', payload: { lang: detectLang, url: remove_hash_url(document.location.href), topics: data.topics } } });
          ajax_put_UrlRecord(mm_user_id(),mm_user_hash(), { href: remove_hash_url(document.location.href), text: t }, function(data) {
            chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'URL_RECORD_SUCCESS', payload: { data: data, url: remove_hash_url(document.location.href) } } });
            chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'PRELOAD_SHARE', payload: { data: data, url: remove_hash_url(document.location.href) } } });
          }, function(error) {
            chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'URL_RECORD_ERROR', payload: { url: remove_hash_url(document.location.href), error: error } } });
          });
          ...
          chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'NLP_INFO_UNKNOWN', payload: { lang: detectLang, url: remove_hash_url(document.location.href), status: false } } });
          ajax_put_UrlRecord(mm_user_id(),mm_user_hash(), { href: remove_hash_url(document.location.href), text: t }, function(data) {
            chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'URL_RECORD_SUCCESS', payload: { data: data, url: remove_hash_url(document.location.href) } } });
            chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'PRELOAD_SHARE', payload: { data: data, url: remove_hash_url(document.location.href) } } });
        ...
        chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'NLP_INFO_ERROR', payload: { lang: detectLang, url: remove_hash_url(document.location.href), error: error, } } });
      });
    } else {
      chrome.extension.sendMessage({ type: 'chromex.dispatch', portName: 'maomao-extension', payload: { type: 'TEXT_NOT_ENGLISH', payload: { lang: detectLang, url: remove_hash_url(document.location.href), } } });
    }
  } else {
    log.warn("%c ** NOT ENOUGH TEXT FOR CALAIS PROCESSING! **", "background:black; color:red; font-weight:bold;");
    //cslib_test_NextYouTubeVid();
    log.info("NOT ENOUGH TEXT -- reseeding...");
    log.warn("%c ** NOT ENOUGH TEXT FOR CALAIS PROCESSING! **", "background:black; color:red; font-weight:bold;");
