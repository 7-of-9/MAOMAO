    ...
    function tabActivated(o) { // why getting object here?! 
        ...
        chrome.tabs.get(tabId, function (new_tab) {
            if (chrome.runtime.lastError)
                ...
                var new_session = session_get_by_tab(new_tab, false);
                if (new_session != null) {
         ...
         chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
             if (chrome.runtime.lastError)
                 ...
                 if (tabs.length > 0) {
                     ...
                     // start TOT for newly focused
                     var new_session = session_get_by_tab(TOT_active_tab, false);
                     if (new_session != null) {
...
//
    chrome.extension.onMessage.addListener(function (message, sender, callback) {
        ...
        if (message == "Rerun script") {
            session = session_get_by_url(sender.window.url);
            if (session != null)
    ...
    function inject_cs(session, tab_id, skip_text) {
        ...
        if (tab_id == null) {
            ...
            chrome.tabs.get(tab_id, function(tab) {
                if (tab != null) {
                    var process = process_url(tab.url); //_.any(ignore_urls_starting_with, function (a) { return tab.url.indexOf(a) == 0 });
                    if (process) {
                        $.each(cs_files, function(ndx, cs) {
