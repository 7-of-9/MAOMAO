...
var nlp_test_run = false;
    ...
    function calais_process(page_meta, nlp) {
        ...
        if (social_tags.length == 0) {
            console.warn("%c ** NO SOCIAL TAGS RETURNED: TODO: need a different way of handling these cases... **", "background:black; color:red; font-weight:bold;");
            return nlp;
        }

        nlp.topic_specific = "?";

        // remove social tags that are also entities -- https://www.youtube.com/watch?v=1WJyEQXZ7dY
        ...
        var tags_exact_title_match = _.filter(social_tags, function(a) {
            //return page_meta.title_ex_stopwords.toLowerCase().split(" ").indexOf(a.name.toLowerCase()) != -1;
        ...
        cslib_info("%c >> nlp.topic_specific = [" + nlp.topic_specific + "] <<", "background:green; color:white; font-weight:bold;");
    ...
    function nlp_calais(page_meta, test_data) {
        var nlp_items = [];
        var nlp = null;
        ...
        $.ajax({
            ...
            crossDomain: true,
            headers: {
                "X-AG-Access-Token": "mhoonLzHshbRvxEntzN69xBuWEv3tKKA",
            ...
            success: function (o) {
                ...
                nlp = { items: nlp_items, topic_general: "?", topic_specific: "?", social_tags: "?" };
                //var table = prettyPrint(nlp, { /*optional options object */ }); //http://james.padolsey.com/javascript/prettyprint-for-javascript/
                //document.body.appendChild(table);

                // post-process nlp data
                calais_process(page_meta, nlp);
            },
