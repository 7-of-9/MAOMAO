...
function set_meta_accessors(meta) { if (meta != null) {
    //delete meta.title;
    //delete meta.image;
...
// gets or creates new session by tab -- does CS injection
function session_get_by_tab(tab, reinject_cs_handlers_on_existing_session) {
    ...
    if (existing_session.length == 1) {
        ...
        if (!session_injected_cs) { // never injected
            console.info("%c >>> get_session_by_tab - existing - never injected [" + session.url + "] >> INJECT AGAIN!", session_style_hi);
            inject_cs(session, tab.id, false);
        } else {
            if (reinject_cs_handlers_on_existing_session)
                inject_cs(session, tab.id, true); // skip injection of text processing again into known session -- just the handlers
        }

    } else if (existing_session.length == 0) {
        if (process_url(tab.url)) {
            // new session (not known in current list) -- inject CS!
            console.info("%c >>> get_session_by_tab - NEW: " + tab.url, session_style_hi);
            session = new_session(url_ex_hash);
            mm.all_sessions.push(session);
            mm_update(session, true);
            inject_cs(session, tab.id, false);
        } else
            console.info("%c (get_session_by_tab - rejecting non-process URL [" + tab.url + "])", "color: gray;");
    } else
        console.error("%c ### get_session_by_tab - GOT >1 URL MATCH! shouldn't happen." + mm.all_sessions.length, "background: red; color: white;");
...
function session_get_by_url(url) {
    ...
    var existing_session = _.filter(mm.all_sessions, function (a) { return a.url == url_ex_hash });
    if (existing_session.length == 1) {
        console.info("%c (get_session_by_url - existing: " + url + ")", "color: gray; ");
        return existing_session[0];
    }
    else if (existing_session.length == 0)
        console.error("%c ### get_session_by_url - GOT NO URL MATCH! shouldn't happen.", "background: red; color: white;");
    else
        console.error("%c ### get_session_by_url - GOT >1 URL MATCH! shouldn't happen.", "background: red; color: white;");
    return null;
}
...
function session_add_view_instance(session) {
    if (session != null) {
        ...
        console.info("%c > NEW SESSION VIEW [" + session.url + "] (view count=" + session.view_timestamps.length + ")", session_style_hi);
        mm_update(session);
    }
}
    ...
    if (session != null) {
        ...
        if (session.TOT_cur_start_at != 0) {
            ...
            if (!isNaN(tot_delta_millis)) {
                ...
                console.info("%c > TOT.stop: delta=" + tot_delta_millis + " (new: " + session.TOT_total_millis / 1000 + " s)" + "[" + session.url + "] sid=" + session.sid, session_style_hi);
            }
            else
...
function session_start_TOT(session) {
    if (session != null) {
        ...
        session.TOT_cur_stop_at = 0;
        console.info("%c > TOT.start... [" + session.url + "] sid=" + session.sid, session_style_hi);
...
function session_update_page_meta(session, page_meta) {
    ...
    session.page_meta = page_meta;
    console.info("%c > PAGE_META " + JSON.stringify(page_meta) + " [" + session.url + "] sid=" + session.sid, session_style_hi);
